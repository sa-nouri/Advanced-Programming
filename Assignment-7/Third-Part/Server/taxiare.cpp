#include "taxiare.h"
using namespace std;
Taxiare::Taxiare(int port):admin("admin","s3cret"),calendar("1-0:0:0"),Server(port){
	credit=0;
}
template <typename T>
string ntoa(T l){
...
string Taxiare::getRequest(string cmd){
	vector<string>command=parseCommand(cmd);

	if(command[0]=="accept_registration"||
		command[0]=="reject_registration"||
		command[0]=="show_registration_requests"||
		command[0]=="set_time"||
		command[0]=="driver_report"||
		command[0]=="passenger_report"||
		command[0]=="system_report")
	
	command.insert(command.begin(),"admin");
...
Driver* Taxiare::findDriver(string userName){
...
Passenger* Taxiare::findPassenger(string userName){
...
User* Taxiare::findCommandingUser(std::string userName){
...
vector<string> Taxiare::parseCommand(string cmd){
...
string Taxiare::registerPassenger(string userName,string password,string phone,string code){
...
string Taxiare::registerDriver(const vector<string>& args){
	for(int i=0;i<drivers.size();i++){
...
string Taxiare::acceptRejectRegistration(std::string driverUserName,bool accept){
	int i;
	bool found=false;
...
string Taxiare::showRegistrationRequests(){
	string result;
	for (int i = 0; i < drivers.size(); i++){
...
string Taxiare::getDiscountCode(Passenger* passenger ){
...
std::string Taxiare::login(string UserName,string pass){
...
std::string Taxiare::logout(User* user){
...
string 	Taxiare::setStatus(Driver* driver,string state,string address){
...
std::string Taxiare::estimateTrip(const std::vector<string>& command){
...
std::string Taxiare::requestTrip(const std::vector<string>& command){
...
bool Taxiare::addToDriversRequests(Trip* trip){
...
void Taxiare::removeFromDriversRequests(Trip* trip){

	for (int i = 0; i < drivers.size(); i++)
		drivers[i]->removeRequest(trip);
}

string Taxiare::cancelTrip(Passenger* passenger){

	if(!passenger->getLastTrip()) return "";
	if(passenger->getLastTrip()->done) return "You don't have undone trip.";
...
string Taxiare::showTripRequests(Driver* driver){
...
string Taxiare::acceptTripRequest(Driver* driver,string passengerUserName){
	
	std::vector<Trip*> requests=driver->getRequests();
	if(driver->getStatus()=="triping")
		return "can not start an other trip till current trip ends.";
	for (int i = 0; i < requests.size(); ++i){
...
string Taxiare::tripStatus(Passenger* passenger){
...
string Taxiare::setTime(std::string newDate){
...
string Taxiare::driverReport(string userName,string beginingDate,string endDate){
...
string Taxiare::passengerReport(std::string userName){
	for (int i = 0; i < passengers.size(); ++i){
		if(passengers[i]->getName()==userName)
...
string Taxiare::systemReport(string beginingDate,string endDate){
...
void Taxiare::on_standard_input(string line){

	if (line == ":q")
		stop();
	string response=getRequest(line);
	cout<<response;
	if(response!="") cout<<endl;

}

void Taxiare::on_new_connection(int identifier) {}

void Taxiare::on_new_message(int identifier, string message){

	vector<string> args=parseCommand(message);

	string response=getRequest(message);
	send(identifier, response);

	if(args.size()==3 &&args[1]=="login" && response=="")
		connectedUsers[args[0]]=identifier;

	if(args.size()==2 && args[1]=="logout" && response=="")
		connectedUsers.erase(args[0]);
}

void Taxiare::on_terminated_connection(int identifier) {

	std::map<std::string, int>::iterator it = serachByIdentity(identifier);

	if(it != connectedUsers.end()){
		getRequest(it->first+" logout");
		connectedUsers.erase(it->first);
	}

}

std::map<std::string, int>::iterator Taxiare::serachByIdentity(int identity){
    
    std::map<std::string, int>::iterator it = connectedUsers.begin();
    while(it != connectedUsers.end())
    {
